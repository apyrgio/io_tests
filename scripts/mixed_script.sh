...
################### LOGGING ##########################
LOGFILE=/home/brainfree/io_tests/log/mixed_`date +%s`.log
(
######################################################
...
dd if=/dev/zero of=/dev/shm/bcache.tmp bs=1M count=1000
...
RES_FILE=bcache_ram_writeback_dio_`date +%s`.ods
